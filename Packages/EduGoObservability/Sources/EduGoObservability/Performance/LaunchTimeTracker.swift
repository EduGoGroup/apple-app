...
public enum LaunchTimeTracker {
    ...
    public static func markAppDelegateStart() {
        guard appDelegateStartTime == nil else { return }
        appDelegateStartTime = Date()
    }

    public static func markFirstFrameRendered() {
    ...
    public static func recordLaunchMetrics() async {
    ...
    public static var totalLaunchTime: Double? {
        guard let start = processStartTime,
              let frame = firstFrameTime else {
            return nil
        }
