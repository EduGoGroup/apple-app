...
struct KeychainServiceTests {
    ...
    func testSaveTokenSuccessfully() async throws {
        // Given
        let token = "test_token_123456"

        // When
        try await sut.saveToken(token, for: testKey)
    ...
    func testSaveTokenOverwritesPreviousValue() async throws {
        // Given
        let firstToken = "first_token"
        let secondToken = "second_token"

        // When
        try await sut.saveToken(firstToken, for: testKey)
        try await sut.saveToken(secondToken, for: testKey)
    ...
    func testSaveMultipleTokensWithDifferentKeys() async throws {
        // Given
        let key1 = "test_key_1"
        let key2 = "test_key_2"
        let token1 = "token_1"
        ...
        // When
        try await sut.saveToken(token1, for: key1)
        try await sut.saveToken(token2, for: key2)

        // Then
        let retrieved1 = try await sut.getToken(for: key1)
        let retrieved2 = try await sut.getToken(for: key2)
        #expect(retrieved1 == token1)
        #expect(retrieved2 == token2)

        // Cleanup
        try await sut.deleteToken(for: key1)
        try await sut.deleteToken(for: key2)
    }

    // MARK: - Get Token Tests

    @Test("Obtener token que no existe retorna nil")
    func testGetNonExistentTokenReturnsNil() async throws {
        // Given
        let nonExistentKey = "non_existent_key_12345"

        // When
        let result = try await sut.getToken(for: nonExistentKey)
    ...
    func testGetExistingTokenReturnsCorrectValue() async throws {
        // Given
    ...
    func testDeleteTokenSuccessfully() async throws {
        // Given
    ...
    func testDeleteNonExistentTokenDoesNotThrow() async throws {
        ...
        try await sut.deleteToken(for: nonExistentKey)
    }

    @Test("Eliminar token múltiples veces no lanza error")
    func testDeleteTokenMultipleTimesDoesNotThrow() async throws {
        // Given
        let token = "token"
        try await sut.saveToken(token, for: testKey)
        ...
        // When & Then
        try await sut.deleteToken(for: testKey)
        try await sut.deleteToken(for: testKey) // Segunda vez no debería fallar
    }
    ...
    func testCompleteLifecycle() async throws {
        ...
        // When - Save
        try await sut.saveToken(token, for: testKey)
        var retrieved = try await sut.getToken(for: testKey)
        ...
        // When - Delete
        try await sut.deleteToken(for: testKey)
        retrieved = try await sut.getToken(for: testKey)
    ...
    func testTokensWithSpecialCharacters() async throws {
        // Given
        let specialToken = "token.with-special_chars@123!#$%"

        // When
        try await sut.saveToken(specialToken, for: testKey)
        let retrieved = try await sut.getToken(for: testKey)
    ...
    @Test("Tokens largos")
    func testLongTokens() async throws {
        // Given
        let longToken = String(repeating: "a", count: 1000)

        // When
        try await sut.saveToken(longToken, for: testKey)
        let retrieved = try await sut.getToken(for: testKey)
