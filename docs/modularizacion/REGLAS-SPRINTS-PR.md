...
Este documento establece las reglas obligatorias para la ejecución de sprints, gestión de ramas, creación de PRs y su ciclo de vida completo para el proyecto Apple (iOS/macOS/visionOS).
...
| **Releases** | Solo crear releases desde `main` (excepto casos especiales documentados) |

### 1.2 Reglas para Paquetes SPM (EduGoFoundation, EduGoDesignSystem, EduGoDomainCore, etc.)

1. **Flujo obligatorio:** El código debe pasar por el flujo completo de PR (feature → dev → main)
2. **Validación multi-plataforma:** Antes de PR, compilar para TODAS las plataformas (iOS, macOS, visionOS)
3. **Versionamiento:** Usar formato `0.x.y` (estamos en desarrollo)
4. **Tests obligatorios:** Cada paquete SPM debe tener tests unitarios pasando

### 1.3 Proyecto Principal (apple-app)
...
| # | Paso | Comando | Paralelizable |
|---|------|---------|---------------|
| 1 | Revisar: planificado vs tracking vs código generado | Manual | No |
| 2 | Actualizar tracking de la tarea | Manual | No |
| 3 | Hacer commit de los cambios | `git commit` | No |
| 4 | Compilar iOS | `./run.sh` | Sí |
| 5 | Compilar macOS | `./run.sh macos` | Sí |
| 6 | Ejecutar tests | `./run.sh test` | Sí |
| 7 | Compilar paquetes SPM | `swift build` en cada paquete | Sí |

**Nota:** Los pasos 4, 5, 6 y 7 pueden ejecutarse en paralelo para optimizar tiempo.
...
|-------------|----------------|
| `feature/*` | `dev` |
| `dev` | `main` (solo cuando se solicite explícitamente) |
...
| Compilación multi-plataforma exitosa (iOS + macOS) | ✅ Sí |
...
Aplica solo cuando se solicite explícitamente.
...
| Escenario | Cuándo crear release |
|----------|---------------------|
| App Store / TestFlight | Solo cuando se solicite explícitamente |
| Tag de versión | Solo cuando se solicite explícitamente |
...
## 7. Comandos de Validación Específicos del Proyecto

### Compilación Multi-Plataforma (OBLIGATORIO antes de PR)
...
### Validación de Paquetes SPM

```bash
# Desde la raíz del proyecto
cd Packages/EduGoFoundation && swift build
cd Packages/EduGoDesignSystem && swift build
cd Packages/EduGoDomainCore && swift build
```
...
**Versión:** 1.0
**Proyecto:** EduGo Apple App (iOS/macOS/visionOS)
